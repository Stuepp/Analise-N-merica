...
def f(x):
    return x * math.sin(6 * math.exp(-x**2))
...
c = (1/(2 * math.pi)) * simps(f, a, b, n)
coeffs.append(c)

for m in range(1, 6):
    am = (1/math.pi) * simps(lambda x: f(x) * np.cos(m * x), a, b, n)
    coeffs.append(am)
    bm = (1/math.pi) * simps(lambda x: f(x) * np.sin(m * x), a, b, n)
    coeffs.append(bm)
...
def g(x, coeffs):
    ...
    for i in range(1, len(coeffs), 2):
        soma += coeffs[i] * np.cos(math.ceil(i/2)*x)
        soma += coeffs[i+1] * np.sin(math.ceil(i/2)*x)
    return soma
